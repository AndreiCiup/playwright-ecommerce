...
test.describe('Product inventory', () => {
  test('displays 6 products after login @smoke @products', async ({ inventoryPage }) => {
    await expect(inventoryPage.productItems).toHaveCount(6);
  });

  test('sort by price low to high @products', async ({ inventoryPage }) => {
    await inventoryPage.sortBy(SORT.priceLH);
    const prices = await inventoryPage.getAllPrices();
    expect(prices).toEqual([...prices].sort((a, b) => a - b));
  });

  test('sort by price high to low @products', async ({ inventoryPage }) => {
    await inventoryPage.sortBy(SORT.priceHL);
    const prices = await inventoryPage.getAllPrices();
    expect(prices).toEqual([...prices].sort((a, b) => b - a));
  });

  test('sort by name A to Z @products', async ({ inventoryPage }) => {
    await inventoryPage.sortBy(SORT.nameAZ);
    const names = await inventoryPage.getAllNames();
    expect(names).toEqual([...names].sort());
  });

  test('sort by name Z to A @products', async ({ inventoryPage }) => {
    await inventoryPage.sortBy(SORT.nameZA);
    const names = await inventoryPage.getAllNames();
    expect(names).toEqual([...names].sort().reverse());
  });

  test('product detail page loads correctly @products', async ({ loggedInPage }) => {
